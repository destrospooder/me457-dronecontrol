...
class EkfAttitude:
    ...
    def propagate_model(self, measurement, state):
        ...
        for i in range(0, self.N):
            ...
            A_d = np.eye(2) + A * self.Ts + np.matmul(A, A) * 1/2 * self.Ts ** 2
            G_d = G * self.Ts
            Q_d = self.Q * self.Ts ** 2
            # update P with discrete time model
            self.P = np.matmul(np.matmul(A_d, self.P), A_d.T) + np.matmul(np.matmul(Q_d, self.Q_gyro), Q_d.T) + Q_d
    ...
    def measurement_update(self, measurement, state):
        ...
        S_inv = np.linalg.inv(self.R_accel + np.matmul(np.matmul(C, self.P), C.T))
        if (y-h).T @ S_inv @ (y-h) < self.gate_threshold:
            L = np.matmul(np.matmul(self.P, C.T), S_inv)
            I2 = np.eye(2)
            self.P = np.matmul(np.matmul((I2 - np.matmul(L, C)), self.P), (I2 - np.matmul(L, C)).T) + \
